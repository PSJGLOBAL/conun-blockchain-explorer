...
import TxnDataBlock from "../../pages/MainPage/TxnDataBlock"

import { TXNTableSkeleton } from "../../ui/Skeletos/MainTableSkeleton"
...
const TransactionTable = ({ txnData, fullPage }: Props) => {
  ...
  if (!loading) {
    if (success) {
      if (txnData && txnData.length > 0) {
        content = txnData.map((i) => (
          <TxnDataBlock key={i.txhash} fullPage={fullPage} data={i} />
        ))
      } else {
        content = (
          <div className={style.noHistory}>No transaction history found.</div>
        )
      }
    } else {
      content = (
